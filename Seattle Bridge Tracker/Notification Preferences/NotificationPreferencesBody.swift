        ...
        ZStack {
            ...
            VStack {
                ...
                Toggle("All Day", isOn: Binding(get: {
                    preferencesModel.preferencesArray.first(where: { $0.id == preference.id })?.isAllDay ?? false
                }, set: { isAllDay in
                    if let index = preferencesModel.preferencesArray.firstIndex(where: { $0.id == preference.id }) {
                        preferencesModel.preferencesArray[index].isAllDay = isAllDay
                    }
                }))
                if let isAllDay = preferencesModel.preferencesArray.first(where: { $0.id == preference.id })?.isAllDay, !isAllDay {
...
struct NotificationPreferencesBody_Previews: PreviewProvider {
    static var previews: some View {
        NotificationPreferencesBody(preference: .defaultPreferences)
    }
}
